  ...
  for (size_t i = 0; i < geometries.size(); ++i) {
    ...
    if (skip_parameterization) {
      if (geo.texture_uvs.empty()) {
        LOG(WARNING) << "Mesh " << i
                     << " has no texture UVs, cannot skip parameterization. "
                        "Skipping mesh.";
        continue;
      }

      // Scale UVs to simulate higher resolution allocation
      std::vector<Eigen::Vector2f> scaled_uvs = geo.texture_uvs;
      for (auto& uv : scaled_uvs) {
        uv *= scale;
      }

      xatlas::UvMeshDecl mesh_decl;
      mesh_decl.vertexUvData = scaled_uvs.data();
      mesh_decl.vertexCount = static_cast<uint32_t>(scaled_uvs.size());
      ...
      if (error != xatlas::AddMeshError::Success) {
        LOG(ERROR) << "Error adding mesh " << i << " to xatlas (AddUvMesh): "
